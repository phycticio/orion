...
$labels = [
	'label_username'             => !empty(get_field( 'label_username', 'option' )) ?
        get_field( 'label_username', 'option' ) : __( 'Username or Email address', 'orion' ),
	'label_password'             => !empty(get_field( 'label_password', 'option' )) ?
		get_field( 'label_password', 'option' ) : __( 'Password', 'orion' ),
	'label_remember'             => !empty(get_field( 'label_remember', 'option' )) ?
		get_field( 'label_remember', 'option' ) : __( 'Remember me', 'orion' ),
	'label_login_link'           => !empty(get_field( 'label_login_link', 'option' )) ?
		get_field( 'label_login_link', 'option' ) : __( 'Login', 'orion' ),
	'label_register_link'        => !empty(get_field( 'label_register_link', 'option' )) ?
		get_field( 'label_register_link', 'option' ) : __( 'Register', 'orion' ),
	'label_forgot_password_link' => !empty(get_field( 'label_forgot_password_link', 'option' )) ?
		get_field( 'label_forgot_password_link', 'option' ) : __( 'Forgot your password?', 'orion' ),
	'label_back_to_block'        => !empty(get_field( 'label_back_to_block', 'option' )) ?
		get_field( 'label_back_to_block', 'option' ) : __( 'Back to %s', 'orion' ),
	'registration_message'       => !empty(get_field( 'registration_message', 'option' )) ?
		get_field( 'registration_message', 'option' ) : __( 'Register For This Site', 'orion' ),
	'login_header_message'       => !empty(get_field( 'login_header_message', 'option' )) ?
		get_field( 'login_header_message', 'option' ) : __( 'Login', 'orion' ),
];
...
?>
<div id="login">
    <div class="orion-login-container">
		...
		global $interim_login, $labels;

		// Don't allow interim logins to navigate away from the page.
		if ( ! $interim_login && get_field( 'show_back_to_blog', 'orion' ) ) {
			?>
            <p id="backtoblog">
				<?php
				$html_link = sprintf(
					...
					/* translators: %s: Site title. */
						_x( $labels['label_back_to_block'], 'site' ),
...
switch ( $action ) {
	...
	case 'confirm_admin_email':
        ...
        <form class="admin-email-confirm-form" name="admin-email-confirm-form"
              action="<?php echo esc_url(
			      site_url
			      (
				      get_field( 'login_url', 'option' ) . '?action=confirm_admin_email',
				      'login_post'
			      )
		      ); ?>" method="post">
			<?php
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ?
					...
					get_field( 'login_url', 'option' ) . '?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		login_header(
			...
			wp_get_admin_notice(
				__( sprintf(
					'Please enter your %s. You will receive an email message with instructions on how to reset your password.',
					strtolower( $labels['label_username'] )
				) ),
				array(
					'type'               => 'info',
					'additional_classes' => array( 'message' ),
        ...
        <form name="lostpasswordform" id="lostpasswordform"
              action="<?php echo esc_url( network_site_url( get_field( 'login_url', 'option' ) . '/?action=lostpassword', 'login_post' ) ); ?>"
              method="post">
            <p>
                <label for="user_login"><?php _e( $labels['label_username'] ); ?></label>
        ...
        <p id="nav">
            <a class="wp-login-log-in"
               href="<?php echo esc_url( wp_login_url() ); ?>"><?php _e( $labels['label_login_link'], 'orion' ); ?></a>
			<?php
			...
			if ( get_option( 'users_can_register' ) ) {
				$registration_url = sprintf(
					...
					esc_url(
						wp_registration_url() ), __( $labels['label_register_link']
					)
				);
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( get_field( 'login_url', 'option' ) . '?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( get_field( 'login_url', 'option' ) . '?action=lostpassword&error=invalidkey' ) );
			}
		...
		if ( ( ! $errors->has_errors() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
			...
			login_header(
				...
				wp_get_admin_notice(
					...
					' <a href="' . esc_url( wp_login_url() ) . '">' . __( $labels['label_login_link'], 'orion' ) . '</a>',
					array(
						'type'               => 'info',
        ...
        <form name="resetpassform" id="resetpassform"
              action="<?php echo esc_url( network_site_url( get_field( 'reset_password_url', 'option' ) . '/', 'login_post' ) ); ?>"
        ...
        <p id="nav">
			...
			if ( get_option( 'users_can_register' ) ) {
				$registration_url = sprintf(
					'<a class="wp-login-register" href="%s">%s</a>', esc_url( wp_registration_url() ),
					$labels['label_register_link']
				);
	...
	case 'register':
		...
		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( get_field( 'login_url', 'option' ) . '?registration=disabled' ) );
			exit;
		}
		...
		if ( $http_post ) {
			...
			if ( isset( $_POST['user_email'] ) && is_string( $_POST['user_email'] ) ) {
                ...
                if(get_field('registration_field_hide_username_field', ORION_SETTINGS_OBJECT_ID)) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : get_field( 'login_url', 'option' ) . '?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
        ...
        <form name="registerform" id="registerform"
              action="<?php echo trailingslashit(esc_url( site_url( get_field( 'register_url', 'option' ), 'login_post' ) ) ); ?>"
              method="post"
              novalidate="novalidate">
            <?php if(!get_field('registration_field_hide_username_field', ORION_SETTINGS_OBJECT_ID)):?>
            <p>
            ...
            <?php endif; ?>
            <p>
                <label for="user_email"><?php _e( 'Email' ); ?> <span class="registration-custom-field-required-mark">*</span></label>
            ...
            <p class="submit">
                <input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large"
                       value="<?php esc_attr_e( $labels['label_register_link'] ); ?>"/>
            </p>
        </form>
        ...
        <p id="nav">
			...
			$html_link = sprintf(
				...
				__( $labels['label_forgot_password_link']
				)
			);
	...
	default:
		...
		if ( $interim_login ) {
			if ( ! $errors->has_errors() ) {
				$errors->add( 'expired', __(
					sprintf(
						'Your session has expired. Please %s to continue where you left off.',
						strtolower( $labels['label_login_link'] )
					)
				),
					'message'
				);
		...
		login_header( __( $labels['login_header_message'] ), '', $errors );
        ...
        <form name="loginform" id="loginform"
              action="<?php echo esc_url( site_url( get_field( 'login_url', 'option' ), 'login_post' ) ); ?>/"
              method="post">
            <p>
                <label for="user_login"><?php _e( $labels['label_username'] ); ?></label>
            ...
            <div class="user-pass-wrap">
                <label for="user_pass"><?php _e( $labels['label_password'] ); ?></label>
                <div class="wp-pwd">
            ...
            <p class="forgetmenot"><input name="rememberme" type="checkbox" id="rememberme"
                        ...
                        for="rememberme"><?php esc_html_e( $labels['label_remember'] ); ?></label></p>
            <p class="submit">
                <input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large"
                       value="<?php esc_attr_e( $labels['label_login_link'] ); ?>"/>
				<?php
            ...
            <p id="nav">
				...
				if ( get_option( 'users_can_register' ) ) {
					$registration_url = sprintf(
						...
						__( $labels['label_register_link'], 'orion' )
					);
